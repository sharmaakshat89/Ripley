...
```bash
INIT=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/workflows/execute-plan.md` **inline**
...
```bash
node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       @/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/references/checkpoints.md
       @/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
PARENT_INFO=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
COMPLETION=$(node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/Users/richardhightower/clients/spillwave/src/agent-brain/.opencode/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
